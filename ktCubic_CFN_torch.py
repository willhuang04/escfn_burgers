...
class KurganovTadmorSchemeTorch(nn.Module):
    ...
    def rhs(self, u: torch.Tensor) -> torch.Tensor:
        ...
        batch_size, _, channels = u.shape
        if self.boundary == "same":
            # Periodic Wrap
            u_padded = torch.cat([u[:, -2:, :], u, u[:, :2, :]], dim=1)
            
        elif self.boundary == "fixed":
            # Dirichlet: Ghost cells are set to the constant boundary values
            # Create ghost cells for the left (u=bcs[0]) and right (u=bcs[1])
            left_val = u[:, 0:1, :]
            right_val = u[:, -1:, :]
            left_ghost = left_val.repeat(1, 2, 1)
            right_ghost = right_val.repeat(1, 2, 1) 
            u_padded = torch.cat([left_ghost, u, right_ghost], dim=1)
            
        else:
            raise NotImplementedError(f"Boundary type {self.boundary} not supported.")
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=["train", "check", "eval"], default="eval")
    parser.add_argument("--train", default="Data/trainData_CubicFlux_Expansion_256.npy")
    parser.add_argument("--val", default="Data/valData_CubicFlux_Expansion_256.npy")
    parser.add_argument("--test", default="Data/testData_CubicFlux_Expansion_256.npy")
