...
def get_Datasets(Noise_level,rng=jax.random.PRNGKey(100), L = 10, data_path = 'Data/report/dirichlet/trainData.npy'):
    """Load Dataset"""
...
def TrainEntropyStableScheme(epochs, lr = 1e-4,ckpt_dir='./ckpts/Edge/'):
    ...
    timeSteps = 20 
    Noise_level = 0
    train_ds = get_Datasets(Noise_level, L=timeSteps, rng = gendata_rng, data_path = 'Data/report/dirichlet/trainData_dirichlet_sq_sin_Burgers_'+str(Nx)+'.npy')
    rng, gendata_rng = jax.random.split(rng)
    val_ds = get_Datasets(Noise_level, L=timeSteps, rng = gendata_rng, data_path = 'Data/report/dirichlet/valData_dirichlet_sq_sin_Burgers_' + str(Nx) + '.npy')
...
def evaluateESS(epochs,data_path, ckpt_dir='ckpts/Edge/'):
    ...
    #Check flux plot
    def plot_flux(state, model):
        # Create input grid
        u_vals = jnp.linspace(-2.0, 2.0, 200)[:, None]  # shape (200, 1)

        # Evaluate neural flux
        f_vals = model.Num_flux.apply(
            {'params': state.params['flux']},
            u_vals
        )

        # Convert to numpy for plotting
        u_vals = np.array(u_vals).flatten()
        f_vals = np.array(f_vals).flatten()

        # Plot
        plt.figure()
        plt.plot(u_vals, f_vals, label='Neural flux')
        
        # Compare with Burgers true flux
        plt.plot(u_vals, 0.5 * u_vals**2, '--', label='0.5 u^2 (true)')
        
        plt.xlabel('u')
        plt.ylabel('f(u)')
        plt.legend()
        plt.title('Neural Flux Function (Burgers)')
        plt.show()
    plot_flux(state, EntropyStableForm)
